...
def elementary_ops_interchange_rows(
    ...
    A[ith_row-1],A[jth_row-1]=A[jth_row-1],A[ith_row-1].copy()
...
def elementary_ops_scale_row(
    ...
    A[ith_row-1]=A[ith_row-1]*scalar_val       
    #--------------------------------------------------------------------------#
    #                             End of your code                             #
    ############################################################################
...
def elementary_ops_change_row(
    ...
    #                             Your code will be here                       #
    #--------------------------------------------------------------------------#
    A[ith_row-1]+=A[jth_row-1]*scalar_val
...
def elementary_ops_interchange_cols(
    ...
    A[:,ith_col-1],A[:,jth_col-1]=A[:,jth_col-1],A[:,ith_col-1].copy()
...
def elementary_ops_scale_col(
    ...
    #                             Your code will be here                       #
    #--------------------------------------------------------------------------#
    A[:,ith_col-1]=A[:,ith_col-1]*scalar_val
...
def elementary_ops_change_col(
    ...
    #                             Your code will be here                       #
    #--------------------------------------------------------------------------#
    A[:,ith_col-1]+=A[:,jth_col-1]*scalar_val
...
def elementary_ops_interchange_cols_mult(
    ...
    I=np.identity(len(A[0]),float)
    I[:,ith_col-1],I[:,jth_col-1]=I[:,jth_col-1],I[:,ith_col-1].copy()
    # print(I)
...
def elementary_ops_scale_col_mult(
    ...
    I=np.identity(len(A[0]),float)
    I[:,ith_col-1]=I[:,ith_col-1]*scalar_val
    A=matrix_multiplication(A,I) 
...
def elementary_ops_change_col_mult(
    ...
    I=np.identity(len(A[0]),float)
    I[:,ith_col-1]+=I[:,jth_col-1]*scalar_val    
    A=matrix_multiplication(A,I) 
...
def reduced_echelon_form(
    ...
    for i in range(len(A[0])):
        for j in range(r,len(A)):
            if A[j][i]!=0:
                k = A[j][i]
                A = elementary_ops_interchange_rows(np.array(A,float), r+1, j+1)
                A = elementary_ops_scale_row(np.array(A,float), r+1, 1/k)
                for l in range(r+1,len(A)):
                    m = A[l][i] 
                    if m != 0:
                        A = elementary_ops_change_row(np.array(A,float), l+1, r+1, m*(-1))
                r+=1
                j = len(A)
                break
    ...
    for i in range(len(A)-1):
        for j in range(i+1,len(A)):
            if A[j][i] != 0:
                r = A[i,i]
                k = A[j,i]
                A = elementary_ops_change_row(np.array(A,float), j+1, i+1, -k / r)
